#!/usr/bin/env bun
// JobPlanet (잡플래닛) — Scrapling fetch (Python venv) + bun CLI wrapper.
...
const HELP = `jobplanet-cli — search jobs on JobPlanet (잡플래닛), South Korea

Requires Scrapling in ../fetch/.venv (run \`bun run setup\` once).

USAGE
  bun run src/cli.ts search --query "<keywords>" [flags]
  bun run src/cli.ts detail <posting_id|url> [--format json|plain]
...
EXAMPLES
  ...
  bun run src/cli.ts search -q "백엔드" --limit 5
  bun run src/cli.ts detail 1500010 --format plain

Personal use only — browser fetch via Scrapling; keep volume low.
`
