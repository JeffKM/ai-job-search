...
function renderTable(cards: JobCard[]): string {
  ...
  const header =
    "ID".padEnd(10) + " " + "TITLE".padEnd(36) + " " + "COMPANY".padEnd(20) + " " + "LOCATION".padEnd(20)
  return [header, "-".repeat(header.length), ...rows].join("\n")
}

export async function runSearch(opts: SearchOpts): Promise<number> {
  try {
    const nativeTypes = opts.companyType ? saraminCompanyTypes(opts.companyType) : undefined
    const html = await htmlFetch(buildSearchUrl(opts.query, opts.page, nativeTypes))
    ...
    const meta = {
      count: cards.length,
      page: opts.page,
      ...(opts.companyType ? companyTypeFilterMeta(opts.companyType, nativeTypes ? "native" : "client") : {}),
    }
