...
const HELP = `jobkorea-cli — search jobs on JobKorea (잡코리아), South Korea
...
EXAMPLES
  ...
  bun run src/cli.ts search -q "데이터 엔지니어" --limit 15
  bun run src/cli.ts search -q "python" --company-type major --format table
  bun run src/cli.ts detail 49546039 --format plain
